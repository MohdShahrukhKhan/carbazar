...
#         put :update, params: { id: user.id, user: { name: 'Updated Name', email: 'updated@example.com' } }
#         expect(response).to have_http_status(:ok)
#         expect(JSON.parse(response.body)['data']['name']).to eq('Updated Name')
#       end

#       it "returns unprocessable_entity with invalid parameters" do
...
#         put :update, params: { id: 9999, user: { name: 'Non-existent User' } }
